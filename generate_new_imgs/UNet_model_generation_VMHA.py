...
class DiffiT_AttentionBlock(nn.Module):
    ...
    def forward(self, x, t):
        ...
        x = self.attention_block(x, t)
        x = x + x_skip1

        x_skip2 = x.clone()
        x = self.batch_norm2(x)
        x = self.swish(self.conv(x))
        x = x + x_skip2
        return x
...
class DiffiT_ResBlock(nn.Module):
    def __init__(self, input_channels, out_channels, time_emb_dim, device=None):
        ...
        self.group_norm = nn.GroupNorm(num_groups=32, num_channels=input_channels)
    ...
    def forward(self, x, t):
        time_emb = self.swish(self.time_mlp(t))
        time_emb = time_emb[(..., ) + (None, ) * 2]
        x_skip = x.clone()
        ...
        x = self.AttentionBlock(x, time_emb)

        return x + x_skip

class Residual_DiffiT_UNet_generation(nn.Module):
    def __init__(self, image_channels=3, out_dim=3, num_classes=None, device=None):
        super().__init__()
        self.image_channels = image_channels
