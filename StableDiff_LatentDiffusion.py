...
class LatentDiffusion_superres:
    ...
    def fine_tuning_Diffusion(self,
        ...
        # unet.eval() ############## TO PERFORM FINETUNING OF THE ViT ENCODER OF THE LOW RESOLUTION IMAGE
        # optimizer = torch.optim.Adam(image_encoder.parameters(), lr=learning_rate) ############## TO PERFORM FINETUNING OF THE ViT ENCODER OF THE LOW RESOLUTION IMAGE
        ...
        # snapshot = image_encoder.state_dict() ############## TO PERFORM FINETUNING OF THE ViT ENCODER OF THE LOW RESOLUTION IMAGE
        # torch.save(snapshot, os.path.join('models_run','ViT_finetuning.pt')) ############## TO PERFORM FINETUNING OF THE ViT ENCODER OF THE LOW RESOLUTION IMAGE
        # print(f"ViT encoder saved at {os.path.join('models_run','ViT_finetuning.pt')}") ############## TO PERFORM FINETUNING OF THE ViT ENCODER OF THE LOW RESOLUTION IMAGE
    ...
    def sample_superres(self, lr_image, num_inference_steps,):
        ...
        device = self.device
        image_encoder = ViTModel.from_pretrained("google/vit-base-patch16-224-in21k").to(device)
        # image_encoder_snapshot_path = os.path.join('models_run','ViT_finetuning.pt') ############## TO PERFORM FINETUNING OF THE ViT ENCODER OF THE LOW RESOLUTION IMAGE
        # if os.path.exists(image_encoder_snapshot_path): ############## TO PERFORM FINETUNING OF THE ViT ENCODER OF THE LOW RESOLUTION IMAGE
        #     print(f"Loading fine-tuned ViT model from {image_encoder_snapshot_path}...") ############## TO PERFORM FINETUNING OF THE ViT ENCODER OF THE LOW RESOLUTION IMAGE
        #     image_encoder.load_state_dict(torch.load(image_encoder_snapshot_path, map_location=device))
...
def launch(args):
    ...
    lr_image = Image.open('celebA_100k/test_original/197268.jpg').resize((image_size//magnification_factor, image_size//magnification_factor))
