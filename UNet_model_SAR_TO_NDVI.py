...
class ResConvBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, time_emb_dim, device):
        ...
        self.conv1 = nn.Sequential(
                                  ...
                                  self.batch_norm1,
                                  self.relu)
        self.conv_SAR_img = nn.Conv2d(in_ch, out_ch, 3, padding=1)
    ...
    def forward(self, x, t, x_skip):
        ...
        if x_skip is not None:
            x_skip = self.conv_SAR_img(x_skip)
            h = h + x_skip
        # TIME EMBEDDING
...
class RRDB(nn.Module):
    '''
    This class is used in order to encode the SAR image.
...
class Residual_Attention_UNet_SAR_TO_NDVI(nn.Module):
    def __init__(self, SAR_channels=2, NDVI_channels=1, device=None):
        ...
        # INITIAL PROJECTION
        self.conv0 = nn.Conv2d(self.NDVI_channels, self.down_channels[0], 3, padding=1) # SINCE THERE IS PADDING 1 AND STRIDE 1,  THE OUTPUT IS THE SAME SIZE OF THE INPUT

        # SAR ENCODER
        self.SAR_encoder = RRDB(in_channels=self.SAR_channels, out_channels=self.SAR_channels, num_blocks=3)

        self.conv_SAR_img = nn.Conv2d(self.SAR_channels, self.down_channels[0], 3, padding=1)
        ...
        # OUTPUT
        self.output = nn.Conv2d(self.up_channels[-2], self.NDVI_channels, 1)
    ...
    def forward(self, NDVI_img, timestep, SAR_img):
        t = timestep.unsqueeze(-1).type(torch.float)
        t = self.pos_encoding(t, self.time_emb_dim, device=self.device)

        # INITIAL CONVOLUTION
        x = self.conv0(NDVI_img)

        # SAR ENCODER
        SAR_img = self.SAR_encoder(SAR_img)

        SAR_img = self.conv_SAR_img(SAR_img)
        
        # SUM THE UP SAMPLED SAR IMAGE WITH THE INPUT IMAGE
        x = x + SAR_img
        x_skip = x.clone()
