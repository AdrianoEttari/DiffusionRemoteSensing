...
from train_diffusion_superres_DIFFUSERS import Diffusion as Diffusion_superres
...
def super_resolver(lr_img, device, model_name, model='Residual Attention Unet'):
        ...
        elif model.lower() == 'diffusers unet':
                model = UNet2DModel(
                sample_size=image_size,  # the target image resolution
                in_channels=input_channels,  # the number of input channels, 3 for RGB images
                out_channels=output_channels,  # the number of output channels
...
if __name__ == '__main__':
       #### SUPER RESOLUTION EXAMPLE #####
        ...
        downsample = transforms.Resize((48, 48),
                                       interpolation=transforms.InterpolationMode.BICUBIC)
        lr_img = transform(lr_img)
        lr_img = downsample(lr_img)
        blur_radius = random.triangular(0.5,1.5,1)
        ...
        model_name = 'Residual_Attention_UNet_superres_magnification4_LRimgsize48_CelebA100k_downblur_DIFFUSERS'
        superres_img = super_resolver(lr_img, device, model_name, model='Diffusers UNet')
        file_name = os.path.basename(img_path)
        save_path = os.path.join('000114_sr.png')
