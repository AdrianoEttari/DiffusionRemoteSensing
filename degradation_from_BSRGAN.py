...
"""
from util of the original code
"""
...
def imread_uint(path, n_channels=3):
    ...
    if np.max(img) > 1.0:
        # Convert image data to the range of 0-1
...
if __name__ == '__main__':
    ...
    sf = 4
    os.makedirs('degradates_patches', exist_ok=True)
    start = time.time()
    for i in range(10):
        ...
        cv2.imwrite(f'./degradates_patches/prova{i}.jpg', img_concat)

        # CV2 expects images in BGR format
        # img_lq = single2uint(img_lq)[:, :, [2, 1, 0]]
        # img_hq = single2uint(img_hq)[:, :, [2, 1, 0]]
        # cv2.imwrite(f'./degradates_patches/lq_{i}.jpg', img_lq)
        # cv2.imwrite(f'./degradates_patches/hq_{i}.jpg', img_hq)
